...
public class StringOpsTest {
	...
	public void testEquals(@From(ABCStringGenerator.class) String s1, @From(ABCStringGenerator.class) String s2) {
		...
		// Use assumeThat to filter for strings with different lengths
		assumeThat(s1.length(), not(equalTo(s2.length())));
		
		// When strings have different lengths, equals should return false
		assertFalse("Strings with different lengths should not be equal", StringOps.equals(s1, s2));
	}
